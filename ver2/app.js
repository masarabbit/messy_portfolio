...
function init() {
  ...
  const spinLastCardToFront = () => {
    if (setting.navigating) return
    const lastCard = setting.sortedImages[0] // last one is the first one in the array
    const { left, top } = lastCard.getBoundingClientRect()
    const otherCards = setting.sortedImages.filter((_card, i) => i > 0)
    setting.sortedImages = [...otherCards, lastCard]
    positionStackedCards()
  ...
  const shuffleCards = () => {
    if (setting.mode !== 'grid') {
      if (isStackMode()) setting.sortedImages = shuffledArray(setting.sortedImages)
      reposition()
    }
  }
  ...
  elements.buttons.forEach(btn => {
    ...
    addClickEvent(btn, 'prev_btn', spinLastCardToFront)
    addClickEvent(btn, 'next_btn', spinBackFrontCardToBack)
  })
}
